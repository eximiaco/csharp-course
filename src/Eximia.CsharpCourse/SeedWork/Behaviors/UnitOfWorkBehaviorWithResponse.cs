...
public class UnitOfWorkBehaviorWithResponse<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : IRequest<TResponse>
{
    private readonly ILogger<UnitOfWorkBehaviorWithResponse<TRequest, TResponse>> _logger;
    ...
    public UnitOfWorkBehaviorWithResponse(
        ILogger<UnitOfWorkBehaviorWithResponse<TRequest, TResponse>> logger,
    ...
    {
        if (request.GetType().GetInterfaces().All(c => c != typeof(ICommand<TResponse>)))
            return await next();
